...
void ImageReportDrawer::drawReport(const ProcessingResults &res, ConfigurationManager *c, const QHash<qint32,bool> &whatToAdd){
    ...
    if (whatToAdd.value(STAT_ID_TOTAL_FIXATIONS)){
        ...
        d.value = QString::number(res.value(STAT_ID_TOTAL_FIXATIONS),'f',0);
        if (c->getBool(CONFIG_DEMO_MODE)) d.value = "439";
        d.stopValues.clear();
        ...
        d.value = QString::number(res.value(STAT_ID_MULTIPLE_FIXATIONS)*100.0/res.value(STAT_ID_TOTAL_FIXATIONS),'f',0);
        if (c->getBool(CONFIG_DEMO_MODE)) d.value = "20";
        d.stopValues.clear();
        ...
        d.value = QString::number(res.value(STAT_ID_FIRST_STEP_FIXATIONS)*100.0/res.value(STAT_ID_TOTAL_FIXATIONS),'f',0);
        if (c->getBool(CONFIG_DEMO_MODE)) d.value = "40";
        d.stopValues.clear();
        ...
        d.value = QString::number(res.value(STAT_ID_SINGLE_FIXATIONS)*100.0/res.value(STAT_ID_TOTAL_FIXATIONS),'f',0);
        if (c->getBool(CONFIG_DEMO_MODE)) d.value = "45";
        d.stopValues.clear();
    ...
    if (whatToAdd.value(STAT_ID_ENCODING_MEM_VALUE)){
        ...
        d.value = QString::number(res.value(STAT_ID_ENCODING_MEM_VALUE),'f',3);
        if (c->getBool(CONFIG_DEMO_MODE)) d.value = "0.00159";
        d.stopValues.clear();
    ...
    if (!c->getBool(CONFIG_REPORT_NO_LOGO)){
        QImage logo(":/report_text/report_text/viewmind.png");
    ...
    QGraphicsTextItem *patient_data = canvas->addText(c->getString(CONFIG_PATIENT_NAME),upperBarInfo);
    patient_data->setDefaultTextColor(QColor(COLOR_FONT_WHITE));
    QGraphicsTextItem *doctor_data  = canvas->addText(c->getString(CONFIG_DOCTOR_NAME),upperBarInfo);
    doctor_data->setDefaultTextColor(QColor(COLOR_FONT_WHITE));
    QGraphicsTextItem *age_data     = canvas->addText(c->getString(CONFIG_PATIENT_AGE),upperBarInfo);
    ...
    QString generatedReport = c->getString(CONFIG_PATIENT_DIRECTORY) + "/" + FILE_REPORT_NAME  + ".png";
...
void ImageReportDrawer::drawSegmentBarLengthsAndIndicators(const ShowDatum &d, qreal yBarLocation, qreal xStart){
    ...
    //if ((d.stopValues.size() == 4) && (d.largerBetter)) colorScale << COLOR_RED << COLOR_YELLOW << COLOR_GREEN;
    //else if ((d.stopValues.size() == 4) && (!d.largerBetter)) colorScale << COLOR_GREEN << COLOR_YELLOW << COLOR_RED;
    //else if ((d.stopValues.size() == 3) && (d.largerBetter)) colorScale << COLOR_YELLOW << COLOR_GREEN;
    //else if ((d.stopValues.size() == 3) && (!d.largerBetter)) colorScale << COLOR_GREEN << COLOR_YELLOW;
    //else return;
    ...
    // OLD CODE: Calculated indicator based on actual scale of the bar.
    //if (vreal <= d.stopValues.first()) indicatorOffset = 0;
    //else if (vreal >= d.stopValues.last()) indicatorOffset = RESULTS_SEGBAR_WIDTH;
    //else indicatorOffset = ((vreal - d.stopValues.first())/fullRange)*RESULTS_SEGBAR_WIDTH;
...
void ImageReportDrawer::loadFonts(){
    ...
    QString base = ":/report_text/report_text/";
...
bool ImageReportDrawer::loadLanguageConfiguration(const QString &lang){
    ...
    QString path = ":/report_text/report_text/";
