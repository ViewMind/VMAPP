...
class PolygonGraph {
   ...
   #LINE_WIDTH = 0.25
   #FONT_SIZE  = 12;
   #LABEL_LEFT_SIDE_MARGIN = 0.01  //% of the bouding squre side.
   #FIXED_DISTANCE_TO_LABEL = 0.02 //% of the bouding squre side.
    ...
    * @param {number} [configuration.levels] - Array of strings for the label for each level in the graph. It's legth is the number of levels. Highest is zero, lowest (closest to center) is configuration.levels.length-1
    * @param {Array}  [configuration.values] - The index correspondig to the level for the vertex in the same index as vertex_labels. Should be a number beteen 0 and configuration.levels.length-1
   ...
   constructor(pdf, configuration){
      ...
      // The we do the fill.
      this.renderFilledGraph(configuration.values)

      // The we render all polygons.
      for (var i = 0; i < this.#vertices.length; i++){
         this.renderPolygon(this.#vertices[i]);
      }
