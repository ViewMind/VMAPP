...
const LayoutParser = {
   ...
   TEXT_FONT_SIZE: 10,
   TEXT_BOX_LINE_WIDTH: 0.5,
   TEXT_BOX_RATIO_FOR_R_VERTICAL: 0.05,
   TEXT_BOX_RATIO_FOR_R_HORIZONTAL: 0.02,
   ...
   parseResultSegment:  function(pdf, item, text_replacer){
      ...
      // Secondary replacement, if exists, is an array of key value pairs to replace the key text for the value text in the title and range text
      if ("second_replacement" in item){
         for (var key in item.second_replacement){
            title = title.replace(key,item.second_replacement[key]);
            range_text = range_text.replace(key,item.second_replacement[key]);
         }
      }
   ...
   parsePlainText: function (pdf,item,text_replacer){
      ...
      pdf.setFontSize(Utils.FontSizes.NORMAL);
      pdf.setFont(Utils.FontNames.NAME,Utils.FontNames.NORMAL);
      pdf.setTextColor(Utils.Colors.TEXT_GRAY);
   ...
   parseTextBox: function (pdf,item,text_replacer){
      ...
      let mandatory = ["id", "text", "x", "y", "w", "h" ,"effective_area"];
      let missing = this.verifyAllKeys(item,mandatory);
      if (missing.length > 0){
         ret.error = "Parse Text: Missing keys " + missing.join(",");
         return ret;
      ...
      // Computing the x, y position. 
      let bb = this.computeItemBoundingBox(item,"Parse Text Box",false);
      if (bb.error != ""){
         ret.error = bb.error;
         return ret;
      }        

      // Gettign the text. either from a direct value or from the language strings. 
      let text = ""
      if (item.text.includes("#")){
         text = text_replacer.reportString(item.text);
      }
      else {
         console.log("Getting the report value for " + item.text)
         text = text_replacer.reportValue(item.text);
         if (text_replacer.getError() != "") text = false;
      }
      if (text === false){
         ret.error = "Parse Text: Getting text - " + text_replacer.getError();
         return ret;
      ...
      // Drawing the box. 
      pdf.setLineWidth(this.TEXT_BOX_LINE_WIDTH);
      //pdf.setDrawColor(Utils.Colors.MAIN_BLUE);
      pdf.setFillColor(Utils.Colors.WHITE)
      var rx = bb.w*this.TEXT_BOX_RATIO_FOR_R_HORIZONTAL;
      var ry = bb.h*this.TEXT_BOX_RATIO_FOR_R_VERTICAL;
      pdf.roundedRect(bb.x,bb.y,bb.w,bb.h,rx,ry,'F');

      // Rendering the text. 
      pdf.text(text,bb.x+bb.w*0.02,bb.y+bb.h*0.02,{baseline: "top"});
      ret.boundingBox = bb;
   ...
   parseFixationPlot: function (pdf,item,text_replacer){
      ...
      // This is used in order to tell the system that the title needs to be colored. 
      if ("use_colored_title" in item){
         if (item.use_colored_title){
            config.use_colored_title = true;
         }
      }
   ...
   parsePresumptiveDiagnosis: function (pdf,item,text_replacer){
      ...
      //console.log("PDIAG . Index is " + item.text_index + " came out with " + v);
      //console.log("PDIAG . TextBase is " + item.text_base);
