...
const LayoutParser = {
    ...
    * @param {Object} item - The object describing the result segment.
   ...
   parseBulletedList: function(pdf,item,text_replacer){
      ...
      if ("dynamic_bullets" in item){
         ...
         let ref_color = item.dynamic_bullets.ref_color;
         ...
         // We now iterate over all the ids in the color code. 
         // console.log("Searching for ref color " + ref_color)

         if (ref_color in item.variable_color_codes){
   ...
   computeIndexes:  function(layout,report){
      ...
      for (value_key in layout.categories) {
         let available_categories = layout.categories[value_key];

         if (!(value_key in report)){
            return "Value key " + value_key + " is not a report variable";
         }

         let report_value = report[value_key];
         ...
         for (category in available_categories){
            ...
            if (label == ""){
               return "Could not find a category for value " + report_value + " for variable " + value_key;
            }

            // Each category has a value for each variable. 
            report.indexes[category][value_key] = label;
